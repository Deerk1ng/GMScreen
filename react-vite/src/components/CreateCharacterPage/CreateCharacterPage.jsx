...
const CreateCharacterPage = () => {
    ...
    const [strength, setStr] = useState(8)
    const [dexterity, setDex] = useState(8)
    const [constitution, setCon] = useState(8)
    const [intelligence, setInt] = useState(8)
    const [wisdom, setWis] = useState(8)
    const [charisma, setCha] = useState(8)
    ...
    const validateData = (currSection) => {
        const error = {}
        if(currSection == 1) {
    ...
    const nextSection = (currSection) => {
        if(validateData(currSection)) {
            setActiveSection(currSection + 1)
        }
    }
    ...
    return (
        <form onSubmit={handleSubmit} className='main-div event-main'>
            ...
            <div hidden={activeSection == 1 ? false : true}>
                <h2 className='third-color' >Origin</h2>
            ...
            <div hidden={activeSection == 2 ? false : true}>
                ...
                <button type='button' onClick={() => setActiveSection(1)}>Back</button>
                <button type='button' onClick={() => nextSection(2)}>Next</button>
            </div>


            <div hidden={activeSection == 3 ? false : true}>
                ...
                <div className='lbl-inp'>
                    ...
                    <input
                        ...
                        className="event-description event-inp"
                        />
                <button type='button' onClick={() => setActiveSection(2)}>Back</button>
                </div>
                <button className='char-bttn' type="submit">Submit</button>
            </div>
        </form>
    )
}
